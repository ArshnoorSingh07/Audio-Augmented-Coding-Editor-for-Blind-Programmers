...
type VoiceKind = 'error' | 'navigation' | 'confirmation' | 'output';
...
interface State {
  ...
  lastSig: LastSig | undefined;
  scopeCache: Map<string, string[]>;
  decorations: vscode.TextEditorDecorationType[];
  voiceServerClose: (() => void) | undefined;
  voiceServerUrl?: string | undefined;
}

const state: State = {
  panel: undefined,
  lastSpoken: '',
  pending: new Map<string, PendingAnnounce>(),
  lastSig: undefined,
  scopeCache: new Map<string, string[]>(),
  decorations: [],
...
type BlindCoderSettings = {
  rate: number;
  pitch: number;
  lang: string;
  quiet: boolean;
  earconMappings: { [event: string]: string };
  announceKeystrokes: boolean;
};

const DEFAULT_SETTINGS: BlindCoderSettings = {
  rate: 1.0,
  pitch: 1.0,
  lang: 'en-US',
  quiet: false,
...
let runtimeSettings: BlindCoderSettings = { ...DEFAULT_SETTINGS };

async function loadSettings(context: vscode.ExtensionContext) {
  try {
    const cfg = vscode.workspace.getConfiguration();
    const rate = cfg.get<number>('blindCoder.voice.rate', DEFAULT_SETTINGS.rate);
    const pitch = cfg.get<number>('blindCoder.voice.pitch', DEFAULT_SETTINGS.pitch);
    const lang = cfg.get<string>('blindCoder.voice.lang', DEFAULT_SETTINGS.lang);
    const quiet = cfg.get<boolean>('blindCoder.quietMode', DEFAULT_SETTINGS.quiet);
    const earconMappings = cfg.get<any>('blindCoder.earconMappings', DEFAULT_SETTINGS.earconMappings);
    const announceKeystrokes = cfg.get<boolean>(
      'blindCoder.announceKeystrokes',
      DEFAULT_SETTINGS.announceKeystrokes
    );
    ...
    const stored = context.globalState.get<Partial<BlindCoderSettings>>('blindCoder.settings');
    if (stored) {
...
async function saveSettings(context: vscode.ExtensionContext, partial: Partial<BlindCoderSettings>) {
...
function sendInitToPanel(webview: vscode.Webview) {
  const payload = {
    type: 'init',
    state: {
...
async function pushUndoEntry(entry: any) {
  const ctx = (global as any).__blindCoderCtx as vscode.ExtensionContext | undefined;
  if (!ctx) return;
  const stack = (ctx.workspaceState.get<any[]>('blindCoder.undoStack') || []);
  stack.push(entry);
  await ctx.workspaceState.update('blindCoder.undoStack', stack);
}

async function tryUndoLastVoiceChange(context: vscode.ExtensionContext): Promise<boolean> {
  const stack = context.workspaceState.get<any[]>('blindCoder.undoStack') || [];
...
function postToPanel(payload: any) {
...
function speak(text: string | string[], _kind: VoiceKind = 'navigation', earcon?: string) {
  if (runtimeSettings.quiet) return;
  state.lastSpoken = Array.isArray(text) ? (text as string[]).join(' ') : (text as string);
  const payload: any = { type: 'speak', text: Array.isArray(text) ? text : text };
  if (earcon) payload.earcon = earcon;
  postToPanel(payload);
}

function speakSeq(lines: string[]) {
  if (runtimeSettings.quiet) return;
  postToPanel({ type: 'speak-seq', seq: lines });
}

function playEarcon(name: 'tap' | 'buzz' | 'knock' | 'blip' | 'glitch' | 'pop') {
  if (!state.panel) return;
  try {
...
function computeScopeStack(doc: vscode.TextDocument, line: number): string[] {
  const IND = (s: string) => {
    const m = s.match(/^(\s*)/);
    return m ? m[1].length : 0;
  };
  const rows = Math.min(line, doc.lineCount - 1);
  const frame: { indent: number; label: string }[] = [];
  for (let i = 0; i <= rows; i++) {
    const t = doc.lineAt(i).text;
    if (!t.trim()) continue;
    const indent = IND(t);
    while (frame.length && frame[frame.length - 1].indent >= indent) frame.pop();
    const label = scopeLabel(t);
    if (label) frame.push({ indent, label });
  }
  return frame.map((f) => f.label);
}

function scopeLabel(t: string): string | undefined {
  const s = t.trim();
  if (s.startsWith('def ')) return 'function';
  if (s.startsWith('class ')) return 'class';
  if (s.startsWith('for ')) return 'for loop';
  if (s.startsWith('while ')) return 'while loop';
  if (s.startsWith('if ')) return 'if block';
  if (s.startsWith('elif ')) return 'elif block';
  if (s.startsWith('else:') || s === 'else') return 'else block';
  return undefined;
}

// ---------- Robust voice server (tries preferred port, small range, persists last port) ----------
async function startLocalVoiceServer(ctx: vscode.ExtensionContext) {
  const PREF_PORT_KEY = 'blindCoder.lastVoicePort';
  const preferredPortFromEnv = Number(process.env.BLINDCODER_VOICE_PORT || 0);
  const storedPort = ctx.globalState.get<number>(PREF_PORT_KEY);
        ...
        if (req.method === 'POST' && req.url === '/transcript') {
          ...
          req.on('end', async () => {
            try {
              ...
              try {
                if ((global as any).__blindCoderDialogInstance && typeof (global as any).__blindCoderDialogInstance.handleTranscript === 'function') {
                  const payloadForDialog: any = {
                    transcript: (json.transcript || json.text || json.transcriptText || '').toString(),
                    isFinal: (typeof json.isFinal === 'boolean' ? json.isFinal : true),
                  };
                  try {
                    (global as any).__blindCoderDialogInstance.handleTranscript(payloadForDialog);
                  } catch (_) {}
                }
              } catch (_) {}
              ...
              if (text) {
                ...
                try {
                  ...
                  if ((global as any).__blindCoderServerTranscriptHandler && typeof (global as any).__blindCoderServerTranscriptHandler === 'function') {
                    (global as any).__blindCoderServerTranscriptHandler(text);
                  }
                } catch (e) {}
  ...
  for (let attempt = 0; attempt < portRange; attempt++) {
    ...
    const listenPromise: Promise<{ port: number; server: http.Server }> = new Promise((resolve, reject) => {
    ...
    try {
      ...
      return { url, close: () => { try { runningServer.close(); } catch (e) {} } };
    } catch (err: any) {
  ...
  try {
    ...
    return { url, close: () => { try { fallbackServer.close(); } catch (e) {} } };
  } catch (e) {
...
class VoiceDialog {
  ...
  private resolveWait: ((t: string | null) => void) | null = null;
  private timeoutHandle: NodeJS.Timeout | null = null;
  private timeoutMs = 18000;

  async startAndAsk(prompt = 'What do you want me to do?'): Promise<string | null> {
    if (this.waiting) {
    ...
    if (!state.voiceServerUrl) {
      try {
        const ctx = (global as any).__blindCoderCtx as vscode.ExtensionContext | undefined;
        if (!ctx) throw new Error('No context to start voice server');
  ...
  handleTranscript(payload: { transcript: string; isFinal?: boolean }) {
    ...
    if (!isFinal) return;
    if (this.resolveWait) this.resolveWait(t || null);
  }

  private waitForTranscript(timeoutMs: number): Promise<string | null> {
    return new Promise((resolve) => {
      this.resolveWait = (t: string | null) => {
        if (this.timeoutHandle) clearTimeout(this.timeoutHandle);
        this.resolveWait = null;
        resolve(t);
      };
      this.timeoutHandle = setTimeout(() => {
        if (this.resolveWait) this.resolveWait(null);
      }, timeoutMs);
    });
  }

  // confirmation helper
  async askForConfirmation(prompt = 'Proceed? Say yes to confirm, or no to cancel.', timeoutMs = 10000): Promise<boolean | null> {
    if (this.waiting) {
      return null;
    }
    this.waiting = true;
    ...
    if (yes.some((v) => r === v || r.startsWith(v + ' ') || r.includes(' ' + v))) return true;
    if (no.some((v) => r === v || r.startsWith(v + ' ') || r.includes(' ' + v))) return false;
    if (r.startsWith('y')) return true;
...
(global as any).__blindCoderDialogInstance = dialogInstance;

// ---------- textToCode (same logic, typed) ----------
function textToCode(spoken: string): { code: string; selectionOffsetLines?: number; feedback?: string } | null {
...
function wrapExprIfNeeded(expr: string) {
  expr = expr.trim();
...
async function insertCode(code: string, selectionOffsetLines?: number) {
...
function parseFixerJson(stderr: string | undefined): any | null {
...
function announceFixerSummary(parsed: any) {
  const parts: string[] = [];
  if (parsed.summary) parts.push(parsed.summary);
  if (Array.isArray(parsed.changes) && parsed.changes.length) {
    for (const c of parsed.changes) {
      if (c.type === 'insert') parts.push(`Inserted ${c.detail || 'text'} at line ${c.line}`);
      else if (c.type === 'delete') parts.push(`Removed ${c.detail || 'text'} at line ${c.line}`);
      else if (c.type === 'replace') parts.push(`Replaced at line ${c.line}`);
      else parts.push(`${c.type || 'Change'} at line ${c.line || '?'}`);
    }
  }
  if (parts.length) speak(parts, 'confirmation', 'tap');
  else if (parsed.summary) speak(parsed.summary, 'confirmation', 'tap');
  else speak('Fix applied.', 'confirmation', 'tap');
}

function compressLineIndicesToRanges(indices: number[]): Array<[number, number]> {
...
function classifyDiagnostic(d: vscode.Diagnostic): ErrorKind {
  const msg = (d.message || '').toLowerCase();
  if (msg.includes('expected') && (msg.includes(':') || msg.includes('colon'))) return 'missingColon';
  if (msg.includes('indentation')) return 'indentation';
  if (msg.includes('unexpected') || msg.includes('invalid syntax')) return 'unexpectedToken';
  if (msg.includes('unmatched') || msg.includes('never closed') || msg.includes('parenthesis') || msg.includes('quote')) return 'unmatchedParen';
  if ((msg.includes('name') && msg.includes('not defined')) || msg.includes('undefined')) return 'undefinedName';
  return 'syntaxError';
}

function earconFor(k: ErrorKind) {
...
function toAnnouncement(d: vscode.Diagnostic, kind: ErrorKind): string {
...
function getWordAt(doc: vscode.TextDocument, pos: vscode.Position): string {
...
function handleDiagnostics(): void {
  ...
  const diags = vscode.languages
    .getDiagnostics(uri)
    .filter((d: vscode.Diagnostic) => d.severity === vscode.DiagnosticSeverity.Error);
  if (!diags.length) {
    state.pending.forEach((p: PendingAnnounce) => clearTimeout(p.timer));
    state.pending.clear();
    return;
  }
  const d = diags[0];
  const sig = `${uri.toString()}:${d.range.start.line}:${d.range.start.character}:${d.message}`;
  if (state.lastSig && state.lastSig.sig === sig && Date.now() - state.lastSig.at < 6000) return;
  const timer = setTimeout(() => {
    const kind = classifyDiagnostic(d);
    const token = getWordAt(editor.document, d.range.start);
    if (kind === 'undefinedName' && token && token.length < 3) return;
...
function handleSelectionChange(e: vscode.TextEditorSelectionChangeEvent) {
  ...
  const left = last.filter((s: string) => !stack.includes(s));
  if (entered.length) speak(`Under ${entered[entered.length - 1]}`, 'navigation');
  if (left.length) speak(`Out of ${left[left.length - 1]}`, 'navigation');
  state.scopeCache.set(key, stack);
}

function handleTyping(e: vscode.TextDocumentChangeEvent) {
  ...
  if (runtimeSettings.announceKeystrokes) {
    for (const change of e.contentChanges) {
      ...
      if (textInserted.length === 1) {
        ...
        else if (ch === '\t') spoken = 'Tab';
        else spoken = ch;
        if (spoken) speak(`Typed ${spoken}`, 'confirmation');
        continue;
      }
  ...
  if (change.text === undefined) return;
  ...
  if (/\s/.test(change.text)) {
    ...
    if (lastWord) speak(lastWord, 'confirmation');
  }
}
...
export function activate(context: vscode.ExtensionContext) {
  ...
  (global as any).__blindCoderCtx = context;
  ...
  function ensurePanel() {
    ...
    try {
      ...
      panel.webview.html = `<html><body><h3 style="color:#ddd">BlindCoder audio — failed to load audioPanel.html</h3><pre style="color:#aaa">${String(
        err
      )}</pre></body></html>`;
    ...
    (global as any).__blindCoderServerTranscriptHandler = async (text: string) => {
      try {
        if (!text) return;
        const parsed = textToCode(text);
        if (parsed) {
        ...
        try {
          ...
          switch (cmd) {
            ...
            case 'shortcutTrigger':
              if (msg.cmd) {
                Promise.resolve(vscode.commands.executeCommand(msg.cmd)).catch((e: any) => {
              ...
              (async () => {
                if (typeof msg.snippet === 'string') {
                  ...
                  try {
                    ...
                    const end = editor.document.lineAt(endLine).range.end;
                    const undoStack = context.workspaceState.get<any[]>('blindCoder.undoStack') || [];
                    undoStack.push({
                      id: Date.now().toString(),
                      timestamp: Date.now(),
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('blindCoder.openAudioPanel', () => ensurePanel()),
    vscode.commands.registerCommand('blindCoder.readCurrentLine', readCurrentLine),
    vscode.commands.registerCommand('blindCoder.readContext', readContext),
    vscode.commands.registerCommand('blindCoder.nextError', () => jumpDiagnostic(+1)),
    vscode.commands.registerCommand('blindCoder.prevError', () => jumpDiagnostic(-1)),
    vscode.commands.registerCommand('blindCoder.summarizeErrors', summarizeErrors),
    vscode.commands.registerCommand('blindCoder.repeatLast', () => speak(state.lastSpoken || '')),
    vscode.commands.registerCommand('blindCoder.whereAmI', whereAmI),
    vscode.commands.registerCommand('blindCoder.voiceDebugHelp', voiceDebugHelp),
    vscode.commands.registerCommand('blindCoder.aiDialog', aiDialog),
    vscode.commands.registerCommand('blindCoder.testBeep', () => playEarcon('tap')),
    vscode.commands.registerCommand('blindCoder.runAndSpeak', readTerminalOutput),
    vscode.commands.registerCommand('blindCoder.fixCodeSyntax', () => fixCodeSyntax(context)),
    ...
    vscode.commands.registerCommand('blindCoder.voiceCode.toggle', () => vscode.commands.executeCommand('blindCoder.voiceCode')),
    vscode.commands.registerCommand('blindCoder.toggleQuiet', async () => {
  ...
  context.subscriptions.push(
    ...
    vscode.workspace.onDidChangeTextDocument(handleTyping)
  );
...
async function fixCodeSyntax(ctx: vscode.ExtensionContext) {
  const editor = vscode.window.activeTextEditor;
  ...
  const childEnv: any = Object.assign({}, process.env);
  for (const k of Object.keys(dot)) {
    childEnv[k] = dot[k];
  }

  exec(`python "${fixerPath}" "${file}"`, { env: childEnv, maxBuffer: 50 * 1024 * 1024 }, async (err, stdout, stderr) => {
    if (err && !stdout?.trim()) {
      speak('Failed to fix code.', 'error', 'glitch');
      if (stderr && stderr.trim()) speak(stderr.trim(), 'error');
    ...
    if (!newText || newText.trim() === oldText.trim()) {
      if (stderr && stderr.trim()) {
        ...
        if (parsedSummary) announceFixerSummary(parsedSummary);
        else speak(stderr.trim(), 'confirmation', 'tap');
      } else {
    ...
    if (parsed) {
      ...
      if (!changedLines.length) {
        ...
        if (stderr && stderr.trim()) speak(stderr.trim(), 'confirmation', 'blip');
      }
    }
  });
}

function loadDotEnvForContext(ctx: vscode.ExtensionContext): Record<string, string> {
  const tryPaths = [
  ...
  editor.document.save().then(() => {
    ...
    exec(`python "${filePath}"`, (err, stdout, stderr) => {
      if (err) {
        ...
        if (stderr && stderr.trim()) speak(stderr, 'output');
        else speak('Process returned an error.', 'output');
      } else {
        speakSeq(['Your code ran successfully.', 'The output is as follows.']);
        if (stdout && stdout.trim()) speak(stdout, 'output');
        else speak('No output produced.', 'output');
      }
    });
  });
}
...
async function summarizeErrors() {
  const ed = vscode.window.activeTextEditor;
  if (!ed) return;
  const diags = vscode.languages.getDiagnostics(ed.document.uri).filter((d: vscode.Diagnostic) => d.severity === vscode.DiagnosticSeverity.Error);
  if (!diags.length) return speak('No errors in this file', 'confirmation');
  const lines = diags.slice(0, 10).map((d: vscode.Diagnostic) => {
...
async function jumpDiagnostic(dir: 1 | -1) {
  const ed = vscode.window.activeTextEditor;
  if (!ed) return;
  const pos = ed.selection.active;
  const diags = vscode.languages.getDiagnostics(ed.document.uri).filter((d: vscode.Diagnostic) => d.severity === vscode.DiagnosticSeverity.Error)
    .sort((a: vscode.Diagnostic, b: vscode.Diagnostic) => a.range.start.line - b.range.start.line || a.range.start.character - b.range.start.character);
  if (!diags.length) return speak('No errors', 'confirmation');
  let idx = diags.findIndex((d: vscode.Diagnostic) => d.range.start.line > pos.line || (d.range.start.line === pos.line && d.range.start.character > pos.character));
  if (dir < 0) idx = idx <= 0 ? diags.length - 1 : idx - 1;
  else idx = idx < 0 ? 0 : idx;
...
async function whereAmI() {
  ...
  if (!stack.length) return speak('Top level', 'navigation');
...
async function voiceDebugHelp() {
  const ed = vscode.window.activeTextEditor;
  if (!ed) return;
  const d = vscode.languages.getDiagnostics(ed.document.uri).find((dd: vscode.Diagnostic) => dd.severity === vscode.DiagnosticSeverity.Error);
  if (!d) return speak('No errors to fix', 'confirmation');
  const p = d.range.start;
...
async function handleVoiceIntent(transcript: string) {
  if (!transcript) {
  ...
  if (/\b(error|what is the error|diagnos|diagnose|why does this fail)\b/.test(s)) {
    ...
    exec(`python -m py_compile "${filePath}"`, (err, _stdout, stderr) => {
      if (!err) {
...
export function deactivate(): void {
  ...
  try {
    if (state.voiceServerClose) state.voiceServerClose();
  } catch {}
  try {
    for (const d of state.decorations) d.dispose();
  } catch {}
}
